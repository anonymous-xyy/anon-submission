...
OUTPUT_DIR = "w3c_accessibility_texts"
DELAY = 1
MAX_PAGES = 150
...
def is_w3c_wai_page(url):
    parsed = urlparse(url)
    if parsed.netloc not in ('www.w3.org', 'w3.org'):
        return False
    if not parsed.path.startswith("/WAI/"):
        return False
    if parsed.path.endswith(('.pdf', '.png', '.jpg', '.jpeg', '.gif', '.svg')):
        return False
    return True
...
def clean_text(soup):
    for element in soup(["script", "style", "nav", "footer", "header"]):
        element.decompose()
    for element in soup.find_all(class_=re.compile(r"(nav|menu|sidebar|toc|breadcrumb|footer)")):
        element.decompose()
...
while to_visit and page_count < MAX_PAGES:
    ...
    try:
        ...
        for link in soup.find_all('a', href=True):
            ...
            if is_w3c_wai_page(absolute_url) and absolute_url not in visited:
